...
public class BTreeFile implements DbFile {
     ...
     * nodes along the path to the leaf node with READ_ONLY permission, and locks the
     * right node with permission perm.
     *
    ...
    private BTreeLeafPage findReverseLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        try {
            if (pid.pgcateg() == BTreePageId.LEAF) {
                ...
                BTreeInternalPage bTreeInternalPage = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
                Iterator<BTreeEntry> iterator = bTreeInternalPage.reverseIterator();

                if(f == null) {
                    return findLeafPage(tid, dirtypages, iterator.next().getRightChild(), Permissions.READ_ONLY, null);
                } else {
                    BTreeEntry bTreeEntry = null;
                    while(iterator.hasNext()) {
                        bTreeEntry = iterator.next();
                        if (f.compare(Op.GREATER_THAN_OR_EQ, bTreeEntry.getKey())){
                            return findLeafPage(tid, dirtypages, bTreeEntry.getRightChild(), Permissions.READ_ONLY, f);
                        }
                    }
                    assert bTreeEntry != null;
	...
	private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages,
													  BTreePageId parentId, Field field) throws DbException, IOException, TransactionAbortedException, NoSuchFieldException, InterruptedException {
	...
	Page getPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm)
			throws DbException, TransactionAbortedException, IOException, NoSuchFieldException, InterruptedException {
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		try {
			int total = sibling.getNumTuples() + page.getNumTuples();
			int pageNumTuples = page.getNumTuples();
			if (page.parent == sibling.parent && page.parent == parent.pid.getPageNumber()) {
				if (isRightSibling) {
					Iterator<Tuple> tuples = sibling.iterator();
					for (int i = 0; pageNumTuples + i < total / 2 && tuples.hasNext(); i++) {
						Tuple tuple = tuples.next();
						sibling.deleteTuple(tuple);
						page.insertTuple(tuple);
					}
					entry.setKey(tuples.next().getField(keyField));
					parent.updateEntry(entry);
				} else {
					Iterator<Tuple> tuples = sibling.reverseIterator();
					for (int i = 0; pageNumTuples + i < total / 2 && tuples.hasNext(); i++) {
						Tuple tuple = tuples.next();
						sibling.deleteTuple(tuple);
						page.insertTuple(tuple);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
											 ...
											 BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException, NoSuchFieldException {
		// some code goes here
		...
		try {
			Iterator<BTreeEntry> left = leftSibling.reverseIterator();
			BTreeEntry right = page.iterator().next();
			BTreeEntry entry = left.next();
			BTreeEntry firstRightEntry = new BTreeEntry(parentEntry.getKey(), entry.getRightChild(), right.getLeftChild());
			page.insertEntry(firstRightEntry);
			left = leftSibling.reverseIterator();
			int total = leftSibling.getNumEntries() + page.getNumEntries();
			for (int i = page.getNumEntries(); i < total / 2 && left.hasNext(); i++) {
				entry = left.next();
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// pointers of all children in the entries that were moved.
		try {
			Iterator<BTreeEntry> right = rightSibling.iterator();
			BTreeEntry left = page.reverseIterator().next();
			BTreeEntry entry = right.next();
			BTreeEntry firstRightEntry = new BTreeEntry(parentEntry.getKey(), left.getRightChild(), entry.getLeftChild());
			page.insertEntry(firstRightEntry);
			right = rightSibling.iterator();
			int total = rightSibling.getNumEntries() + page.getNumEntries();
			for (int i = page.getNumEntries(); i  < total / 2 && right.hasNext(); i++) {
				entry = right.next();
				// 注意这里删的是哪一边的child
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		try {
			Iterator<Tuple> right = rightPage.iterator();
			while (right.hasNext()) {
			...
			if (rightPage.getRightSiblingId() != null) {
				BTreePageId bTreePageId = rightPage.getRightSiblingId();
				BTreeLeafPage bTreeLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, bTreePageId, Permissions.READ_WRITE);
				leftPage.setRightSiblingId(bTreePageId);
	 ...
	 * @throws TransactionAbortedException
	 */
	BTreeRootPtrPage getRootPtrPage(TransactionId tid, HashMap<PageId, Page> dirtypages) throws DbException, IOException, TransactionAbortedException, NoSuchFieldException, InterruptedException {
		synchronized(this) {
			if(f.length() == 0) {
...
/**
 * Helper class that implements the Java ReverseIterator for tuples on a BTreeFile
 */
class BTreeFileIterator extends AbstractDbFileIterator {
	 ...
	 */
	@Override
	protected Tuple readNext() throws TransactionAbortedException, DbException, IOException, NoSuchFieldException, InterruptedException {
...
class BTreeSearchIterator extends AbstractDbFileIterator {
	...
	protected Tuple readNext() throws TransactionAbortedException, DbException,
			NoSuchElementException, NoSuchFieldException, IOException, InterruptedException {
		while (it != null) {
...
/**
 * Helper class that implements the DbFileReverseIterator for search tuples on a
 * B+ Tree File
 */
